...
def check_auth(request):
    if ENABLE_BASIC_AUTH is False:
        return True
    try:
        if request.authorization['username'] == BASIC_AUTH_USER and request.authorization['password'] == BASIC_AUTH_PASS:
            return True
    except Exception:
        return False
...
def read_config():
    ...
    RESTRICT_DOMAIN = config['DOMAIN']['RESTRICT_DOMAIN']
    ALLOWED_DOMAINS = config['DOMAIN']['ALLOWED_DOMAINS']

    VERIFY_EMAIL = config['EMAIL_VERIFICATION']['VERIFY_EMAIL']
    SMTP_HOST = config['EMAIL_VERIFICATION']['SMTP_HOST']
    SMTP_PORT = config['EMAIL_VERIFICATION']['SMTP_PORT']
    SMTP_USER = config['EMAIL_VERIFICATION']['SMTP_USER']
    SMTP_PASSWORD = config['EMAIL_VERIFICATION']['SMTP_PASSWORD']
    SMTP_SENDER = config['EMAIL_VERIFICATION']['SMTP_SENDER']
    SMTP_STARTTLS = config['EMAIL_VERIFICATION']['SMTP_STARTTLS']
    DB_PATH = config['DATABASE']['DB_PATH']

    ENABLE_BASIC_AUTH = config['AUTH']['ENABLE_BASIC_AUTH']
    BASIC_AUTH_USER = config['AUTH']['BASIC_AUTH_USER']
    BASIC_AUTH_PASS = config['AUTH']['BASIC_AUTH_PASS']
...
class WKDFileStore(object):
    ...
    def add(email, armored_key):
        ...
        with open(os.path.join(self.path, zb32zb32filename, "wb")) as f:
...
def add_key():
    ...
    if RESTRICT_DOMAIN:
        if key_inspector.is_valid_domain(ALLOWED_DOMAINS) is False:
